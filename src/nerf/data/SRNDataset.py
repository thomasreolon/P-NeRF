...
class SRNDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        if False and is_chair and stage == "train":
        ...
        self.fixed_model = 0 # or None


    def __len__(self):
        if self.fixed_model: return 1
        return len(self.intrins)

    def __getitem__(self, index):
        if self.fixed_model: index = self.fixed_model
        intrin_path = self.intrins[index]    # index = number of folder (fix it to use fifferent cars)
        ...
        for rgb_path, pose_path in zip(rgb_paths, pose_paths):
            # print(rgb_path)  #  diversi folders ui4h3iuh4234h32, 3j4i234i23n432  
            # batch size=4    --> 4 folders
