...
sys.path.insert(
    0, os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "nerf"))
)
...
class PixelNeRFTrainer(trainlib.Trainer):
    ...
    def calc_losses(self, data, is_train=True, global_step=0):
        if "images" not in data:
            return {}
        all_images = data["images"].to(device=device)  # (SizeBatch, NumViews, 3, H, W)
        ...
        for obj_idx in range(SB):
            ...
            )  # (NV, H, W, 8)  [ 8 = 3+3+1+1  cam_centerx3, caydirx3, z_near, zfar ]
